...
void SetupGui(WINDOW_DATA &windowData,unsigned int initialWindowWidth, unsigned int initialWindowHeight){
    ...
    infoString="No file loaded. Open a chardata_X.bin file to begin.";
        ...
        0))
    {
        ...
        if (nk_menu_begin_label(ctx,"FILE",NK_TEXT_LEFT,nk_vec2(100, 100))){
            ...
            if(nk_menu_item_label(ctx, "OPEN", NK_TEXT_LEFT)){
                std::string filename=OpenFilename("YGO LOTD LE Character Data Files (*.bin)\0*.*\0");
                if (!filename.empty()){
        ...
        if (fileOpen)
            comboLabel=std::to_string(fileData.field1[selectedChar])+" - "+fileData.string1[selectedChar];
        if(nk_combo_begin_label(ctx, comboLabel.c_str(),nk_vec2(237,190),&comboWasClicked)){
            if (comboWasClicked){
                nk_popup_set_scroll(ctx,0,selectedChar*(ctx->current->layout->row.min_height+ctx->style.window.spacing.y));
            }
            if (fileOpen){
                nk_layout_row_dynamic(ctx, 0, 1);
                for (Long i=0; i< fileData.characterCount; i++){
        ...
        if(nk_group_begin_titled(ctx, "group_fields", "Fields", NK_WINDOW_TITLE|NK_WINDOW_BORDER)){
            ...
            nk_label(ctx,"Field 6",NK_TEXT_LEFT);
            nk_label(ctx,"Challenge Arena ID",NK_TEXT_LEFT);
            nk_label(ctx,"Field 8",NK_TEXT_LEFT);
            nk_group_end(ctx);
        }
        ...
        if(nk_group_begin_titled(ctx, "group_hex", "Hex", NK_WINDOW_TITLE|NK_WINDOW_BORDER)){
            if (fileOpen){
                nk_layout_row_dynamic(ctx, 0, 1);
                nk_label(ctx,IntToHexString(fileData.field1[selectedChar]).c_str(),NK_TEXT_LEFT);
                nk_label(ctx,IntToHexString(fileData.field2[selectedChar]).c_str(),NK_TEXT_LEFT);
                nk_label(ctx,IntToHexString(fileData.field3[selectedChar]).c_str(),NK_TEXT_LEFT);
                nk_label(ctx,IntToHexString(fileData.field4[selectedChar]).c_str(),NK_TEXT_LEFT);
                nk_label(ctx,IntToHexString(fileData.field5[selectedChar]).c_str(),NK_TEXT_LEFT);
                nk_label(ctx,IntToHexString(fileData.field6[selectedChar]).c_str(),NK_TEXT_LEFT);
                nk_label(ctx,IntToHexString(fileData.field7[selectedChar]).c_str(),NK_TEXT_LEFT);
                nk_label(ctx,IntToHexString(fileData.field8[selectedChar]).c_str(),NK_TEXT_LEFT);
            }
            nk_group_end(ctx);
        }
        ...
        if(nk_group_begin_titled(ctx, "group_edit", "Edit", NK_WINDOW_TITLE|NK_WINDOW_BORDER)){
            if (fileOpen){
                nk_layout_row_dynamic(ctx, 0, 1);
                nk_label(ctx,std::to_string((int)(fileData.field1[selectedChar])).c_str(),NK_TEXT_LEFT);
                int seriesSelection=fileData.field2[selectedChar];
                    ...
                    for (Long i=0; i< seriesStrings.size(); i++){
                        ...
                        if (nk_selectable_label(ctx,seriesStrings[i].c_str(),NK_TEXT_LEFT,&selected)){
                            ...
                            if (seriesSelection>=seriesStrings.size()-1)
                                fileData.field2[selectedChar]=0xFFFFFFFF;
                            else
                                fileData.field2[selectedChar]=i;
                            nk_combo_close(ctx);
